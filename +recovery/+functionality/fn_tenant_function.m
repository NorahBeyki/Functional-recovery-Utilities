...
Electrical = zeros(num_reals,1);
Water = zeros(num_reals,num_units);
utility_repair_day = zeros(num_reals,num_units);
...
for tu = 1:num_units
    ...
    if unit.is_elevator_required
        ...
        for i=1:num_reals
            %for tu = 1:height(tenant_unit)
            if tenant_units.occupancy_id(tu)~= 1  %means when it is not commercial building
                if utilities.electrical.state(i) == "SEQ1" || utilities.electrical.state(i) == "SEQ2" %For non-commercial, LQ service of elevators meets tenant requirements
                    Electrical(i) = utilities.electrical.NF(i);
                else
                    Electrical(i) = utilities.electrical.FULL(i);
                end
            else
                if utilities.electrical.state(i) == "SEQ1"                   %for commercial offices, LQ service of elevators does not meet tenant requirements
                    Electrical(i) = utilities.electrical.NLF(i);
                elseif utilities.electrical.state(i) == "SEQ2"
                    Electrical(i) = utilities.electrical.NF(i);
                elseif utilities.electrical.state(i) == "SEQ3"
                    Electrical(i) = utilities.electrical.LF(i);
                else
                    Electrical(i) = utilities.electrical.FULL(i);
                end
            end
        ...
        power_supply_recovery_day = max(max(system_operation_day.building.elevator_mcs,system_operation_day.building.electrical_main),Electrical);
    ...
    tenant_sys_recovery_day = max(repair_complete_day .* damage.fnc_filters.water_unit,[],2); 
            ...
            for i=1:num_reals
                ...
                if tenant_units.occupancy_id(tu)== 1  %means when it is commercial building
                    if utilities.water.state(i) == "SEQ1" || utilities.water.state(i) == "SEQ2" %For commercial, LQ service of plumbing meets tenant requirements
                        Water(i,tu) = utilities.water.NF(i);
                    else
                        Water(i,tu) = utilities.water.FULL(i);
                    end
                else
                    if utilities.water.state(i) == "SEQ1"                   %for residential (multi or single unit), LQ service of plumbing does not meet tenant requirements
                        Water(i,tu) = utilities.water.NLF(i);
                    elseif utilities.water.state(i) == "SEQ2"
                        Water(i,tu) = utilities.water.NF(i);
                    elseif utilities.water.state(i) == "SEQ3"
                        Water(i,tu) = utilities.water.LF(i);
                    else
                        Water(i,tu) = utilities.water.FULL(i);
                    end
                end
    ...
    recovery_day.water_potable(:,tu) = max(max(system_operation_day.building.water_potable_main,tenant_sys_recovery_day),Water(:,tu));
    ...
    if unit.is_electrical_required
        ...
        for i=1:num_reals
            %for tu = 1:height(tenant_unit_list)
            if tenant_units.occupancy_id(tu)~= 1  %means when it is not commercial building
                if utilities.electrical.state(i) == "SEQ1" || utilities.electrical.state(i) == "SEQ2" %For non-commericial, LQ service of electrical meets tenant requirements
                    utility_repair_day(i,tu) = utilities.electrical.NF(i);
                else
                    utility_repair_day(i,tu) = utilities.electrical.FULL(i);
                end
            else
                if utilities.electrical.state(i) == "SEQ1"                   %for commercials, LQ service of electricals does not meet tenant requirements
                    utility_repair_day(i,tu) = utilities.electrical.NLF(i);
                elseif utilities.electrical.state(i) == "SEQ2"
                    utility_repair_day(i,tu) = utilities.electrical.NF(i);
                elseif utilities.electrical.state(i) == "SEQ3"
                    utility_repair_day(i,tu) = utilities.electrical.LF(i);
                else
                    utility_repair_day(i,tu) = utilities.electrical.FULL(i);
                end
            end
        ...
        recovery_day.electrical = max(recovery_day.electrical,utility_repair_day(:,tu));
